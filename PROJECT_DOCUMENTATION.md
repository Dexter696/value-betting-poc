...
**Purpose of this document:** a complete, verifiable account of what this system does, how it does it, and why every non-obvious design decision was made — written for audit by another LLM or a human reviewer who has no prior context. §1–§18 describe the legacy v1 codebase and dataset as of commit `a9fe263` (2026-07-25) and are kept as an accurate historical record, not updated as v1 changes (it doesn't — see §0). §0 is the current, actively-maintained section: it is accurate as of commit `43d1420` (2026-07-29) and is the section to read for the system's real, present-day state. Where something is a known limitation or an open question, it is stated as such rather than glossed over.

---

## 0. Legacy dataset notice + remediation status (read this first)

### 0.1 The audit and its verdict

**An independent external audit examined this project on 2026-07-25** (commit `e0a0e12ed0d6c7ff887066a47d0f630e2efff3e1`, database SHA-256 `7fe5770104cbfcf974174210bacc1cd8d8bfc4ceb87afbeb08c8c995e07ad504`, full text at `audit 7-26/value-betting-system-audit-2026-07-25.md`) and returned a **NO-GO verdict** for real money and for any public claim of proven positive ROI. Beyond this document's own §14 bug history, the audit found: a confirmed case where an opportunity re-crossing after a process restart could silently overwrite an earlier opportunity's recorded history (16 of 175 opportunities), a missing freshness/skew gate letting the pipeline pair arbitrarily stale odds, and a Method B evaluation that checked the wrong entry point (only whether B's edge was already above threshold at Method A's entry snapshot, never scanning for B's own first crossing). The audit's own §6.1 states plainly: **23 of the 59 settled Method-A legs entered before this project's first commit, and all 59 entered before the major duplicate-fix commits — none of the current ROI figures are attributable to the code as it exists today.**

The audit proposed a 21-item finding list (F-01 through F-21) and an 8-phase remediation roadmap (Phase 0 through Phase 7). **§0.3 is the phase-by-phase status**, including which findings were fixed outside the phase numbering and which two remain open. A separate document, `audit 7-26/remediation-response-2026-07-29.md`, is written specifically for the auditor's second-pass review — it goes finding-by-finding (all 21) rather than phase-by-phase, and is the better entry point if you're cross-checking against the original audit directly.

### 0.2 Phase 0 — freeze the legacy dataset (DONE, 2026-07-25)

1. The exact database the audit examined is archived read-only and immutably at GitHub release [`legacy-development-2026-07`](https://github.com/Dexter696/value-betting-poc/releases/tag/legacy-development-2026-07) — never overwritten, unlike the `db-sync` release the daily capture cycle keeps replacing.
2. The live dashboard shows a permanent banner: *"Legacy development data — not attributable to current strategy."*
3. Every dashboard build is tagged with `experimentId: legacy-development-2026-07`, embedded in the data itself.
4. **No historical row is retroactively edited under the guise of a "fix."** Corrections are new rows/versions, never silent overwrites.
5. §1–§18 below describe **the legacy dataset and the codebase that produced it** — accurate as a record of what was built and why, not a claim that the resulting numbers demonstrate a working strategy.

### 0.3 Remediation status by phase (as of 2026-07-29, commit `43d1420`)

| Phase | Status | One-line summary |
|---|---|---|
| 0 — Freeze legacy dataset | **DONE** | See §0.2 above. |
| 1 — Schema v2, append-only persistence | **DONE** | 16 new tables, UUID identity (F-02), freshness gate (F-01), Method A/B independence (F-04). |
| 2 — VPS migration | **DECLINED by project owner, 2026-07-29** | Reviewed `VPS_MIGRATION_PLAN.md`, chose to stay on GitHub Actions' free tier. Not a gap — a decision. |
| 3 — Canonical event/market matching | **CORE DONE; calibration blocked on data** | Real bipartite matching live (F-14). No naturally-occurring negative example exists yet to calibrate the auto-accept threshold against. |
| 4 — Fair probability + second benchmark | **Fair-probability DONE; Betfair built but NOT PURSUED, 2026-07-29** | De-vig methods live (F-12/F-13). Betfair client fully built and tested but deliberately not activated — project owner's decision, not a technical blocker (see §0.5). |
| 5 — Online entry/execution model | **DONE** | Entry-policy state machines, idempotent execution, exposure limits, closing/CLV — all live. |
| 6 — Settlement evidence + evaluation | **DONE** | Versioned settlement evidence (F-17), real evaluation reports, wired into the live daily cron (F-06). |
| 7 — Pre-registered experiment | **DONE — a real protocol is frozen** | Method A, 200-event stopping rule, existing exposure limits. Frozen live 2026-07-29 (details in §0.4). |

**Findings (F-01 through F-21) fixed outside the phase numbering above**, found via a full re-verification pass against current code rather than trusting the roadmap's own phase summaries: **F-03** (episode close time was stamped with the last stale odds reading instead of the real close time — audit's own reproduction found 43/44 real `event_started` closes ending before their own kickoff in the legacy dataset), **F-11** (database merge tool silently favored whichever side happened to be `dest` on conflict, risking both stale-data resurrection and lost human review decisions), **F-16** (a total capture failure could look green in CI — no exit code ever reflected it), **F-20** (`requirements.txt` was unpinned). All fixed and verified live; see §0.6 for exact commits and evidence.

**F-07** (GitHub Actions cadence/concurrency isn't truly continuous) and **F-21** (the v1 dashboard's pre-entry chart isn't a strict historical replay) remain open. F-07 is the direct consequence of the Phase 2 decision above — fixing it means the VPS migration, which isn't happening now. F-21 is scoped entirely to the v1 legacy dashboard path (§0.2 above), which isn't receiving further investment.

### 0.4 Phase 7 in detail — the frozen experiment protocol

`vb/protocol.py`'s `freeze_protocol()`/`require_active_protocol()`, backed by a new insert-only `experiment_protocol` table (mirrors `SettlementVersion`'s supersede-not-edit pattern — a later freeze under the same name marks the previous one `superseded_by`, never edits it). Values were confirmed with the project owner directly, not invented by the assistant:

- **Method A only** (raw-v1, ≥3% edge) — the `StrategyDefinition` already running continuously in shadow mode, resolved via the same content-addressed row, not a parallel one.
- **Stopping rule**: 200 distinct settled canonical events, or manual stop, whichever first.
- **Exposure limits**: reused from the existing shadow config (`max_stake_per_event=3.0`, `max_stake_per_site=25.0`).
- **Fair model**: `raw-v1` — the benchmark's own implied probability, no de-vig (`vb.edge.raw_edge`), i.e. Method A exactly as defined throughout this project.
- **Execution haircut**: 1.0s, matching `vb.decision_runner.SHADOW_LATENCY`.
- **Primary metric**: flat net ROI from verified/accepted paper executions, clustered by canonical event, after slippage and known costs (audit §18.1, verbatim).
- **Secondary metrics**: mean/median CLV_fair, 95% event-cluster CLV interval, quote verification rate, acceptance/fill rate, odds slippage, freshness/skew reject rate, profit by site/market/odds bucket, max drawdown, event exposure (audit §18.2).

`scripts/start_experiment.py` froze this for real against the live production database: **protocol id `4a94ef09-b8e6-4cdd-b7c2-b8e590fc1f4f`, `frozen_at` 2026-07-29T12:59:53Z** — verified to survive a full restore-from-cache/capture/backup cycle intact (a fresh export after the freeze showed the row unchanged, `superseded_by` still NULL).

Method A's strategy_version had already been accumulating shadow decisions for days before this freeze (infrastructure testing, not results-driven parameter tuning — the threshold/limits are unchanged since Phase 5). Those earlier decisions are **not** confirmatory data: `vb.evaluation_runner.run_evaluation(..., since=frozen_at)` is the code-level enforcement of that boundary, not just prose in the protocol's `start_rule`. No recurring confirmatory-scoped report is wired up yet (the daily `run_daily_evaluation()` still reports on full shadow history) — generating one is a single `since=` call away whenever wanted.

### 0.5 What is explicitly NOT being pursued right now, and why

Two items were deliberately built to completion (or near it) and then **not activated**, by the project owner's explicit choice rather than a technical blocker:

- **Betfair Exchange integration** (`vb/sources/betfair.py`, built 2026-07-29). A real, tested client against Betfair's Exchange JSON-RPC API — interactive login, `listMarketCatalogue`/`listMarketBook`, MATCH_ODDS parsing, session-expiry retry, and `effective_odds_after_commission()` for the exchange's win-only net-winnings commission model. **Not wired into `scheduled_run.py`.** Two reasons, confirmed with the project owner 2026-07-29: (1) it needs a real Betfair account and `BETFAIR_APP_KEY`/`USERNAME`/`PASSWORD` — account creation and credential handling are outside what the assistant can do regardless of authorization; (2) whether Betfair should act as a *second benchmark* (cross-validating Pinnacle — the audit's literal Phase 4 ask) or as another *comparison* book is a real edge-detection semantics decision that market_key/episode-tracking currently assume is single-benchmark. **This is not a priority right now and is not being pursued further.** If revisited later, wiring is a single `scheduled_run.py` capture step following the existing `PinnacleClient`/`SwisslosClient` pattern, once credentials exist and the benchmark-vs-comparison decision is made.
- **VPS migration** (Phase 2, `VPS_MIGRATION_PLAN.md`, researched 2026-07-25, declined 2026-07-29). The project owner reviewed the full cost/benefit writeup (recurring $10-20/mo, operational burden, vs. removing the two-independent-databases risk and GitHub Actions' 65-minute job ceiling / best-effort cron timing) and chose to keep running on GitHub Actions' free tier while the current setup keeps working. **This is not a priority right now and is not being pursued further.** F-07 stays open as the direct consequence. Revisit only if GitHub Actions' known limitations actually start causing real problems, not preemptively.

Everything else the original audit asked for — schema v2, canonical matching, fair-probability de-vig, execution/exposure tracking, settlement evidence, evaluation reporting, and the pre-registered experiment protocol — is built, wired into the live pipeline, tested, and verified against production. The only other open item, Phase 3's auto-accept-threshold calibration, is blocked on data (no natural negative example in the review queue yet), not on more engineering.

### 0.6 Self-review discipline

A high-effort, 8-finder-angle code review (3 correctness angles + 3 cleanup angles + altitude + conventions, each surfacing up to 6 candidates, independently verified) was run against this whole remediation pass's diff on 2026-07-29 — not requested by the project owner, run proactively before considering the work "done," matching the same discipline §14.6 documents for the original v1 codebase. It found 8 real, confirmed issues in the fixes above, all resolved same-day:

- `capture.yml`'s F-16 exit(1) would have caused GitHub Actions to **skip the DB backup and dashboard-deploy steps** on exactly the day a total capture failure happens (no `if: always()`) — directly undermining F-08's own backup guarantee. Fixed.
- F-16's exit code only reflected capture success, not a full `run_pipeline`/`run_pipeline_v2` crash despite capture succeeding. Broadened to cover both.
- `merge_databases.py`'s `raw_event` conflict-detection omitted `sport` from its own change-detection WHERE clause (a sport-only correction would silently never apply) — directly contradicting the fix's own "source always wins" claim. Fixed.
- `merge_databases.py`'s `event_match_review` merge never reconciled `first_seen_at` — same lossy-merge failure mode F-11 exists to close, on a different column. Fixed.
- `episode.py`'s F-03 fix decided observation reuse-vs-record by comparing timestamps, not real snapshot identity — two genuinely different snapshot pairs sharing a timestamp would have been wrongly conflated. Replaced with a lookup by the real `(episode_id, benchmark_snapshot_id, comparison_snapshot_id, edge_model)` key.
- `betfair.py` didn't retry once on an expired session mid-scrape (dormant code, no live credentials yet, but a real gap once activated). Fixed.
- `content_hash_bytes(DB_PATH.read_bytes())` loaded the entire (350MB+ and growing) capture database into memory once daily just to hash it. Replaced with a streaming `content_hash_file()`.

8 new regression tests added; full suite (334 tests as of this writing) green; every fix verified against a real `capture.yml` run, not just locally.
