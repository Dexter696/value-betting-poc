...
- **Phase 6 (settlement evidence + evaluation, core landed).** `vb/settlement_evidence.py` records versioned `ResultEvidence`/`SettlementVersion` rows for every real settlement, bootstrapped onto a per-source-event `canonical_event` (a deliberate placeholder for the full cross-site fusion Phase 3's matching will eventually provide once calibrated). `vb/evaluation_runner.py` + `vb/evaluation_v2.py` assemble real `ExecutedBet` rows and produce a real evaluation report — run for the first time against live data on 2026-07-29 (38 decisions, 28 unique events). Raw source-response archiving and the dashboard JS rewrite are the two Phase 6 pieces still deliberately deferred (the former touches every scraper's live HTTP path; the latter has little to render yet).
- **Phase 2 (VPS) and Phase 7 (the actual pre-registered experiment)** have not started — both are blocked on infrastructure and protocol decisions only the project owner can make, not on more code.
