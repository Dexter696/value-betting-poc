...
- **Self-review, 2026-07-29.** A high-effort, 8-finder-angle code review of this whole session's diff (not asked for by the project owner - run proactively before considering the session's work "done") found 8 real, confirmed issues in the fixes above: `capture.yml`'s F-16 exit(1) would have skipped the DB backup/dashboard-deploy steps on exactly the day a total capture failure happens (no `if: always()` - now added); F-16's exit code only reflected capture success, not a full `run_pipeline`/`run_pipeline_v2` crash (now covers both); `merge_databases.py`'s `raw_event` conflict WHERE clause omitted `sport` from its own change-detection (a sport-only correction would silently never apply) and `event_match_review` never reconciled `first_seen_at` (same lossy-merge failure mode F-11 exists to close, on a different column); `episode.py`'s F-03 fix decided observation reuse-vs-record by comparing timestamps rather than real snapshot identity (two different snapshot pairs sharing a timestamp would have been wrongly conflated) - replaced with a lookup by the actual `(episode_id, benchmark_snapshot_id, comparison_snapshot_id, edge_model)` key; `betfair.py` didn't retry once on an expired session mid-scrape; `content_hash_bytes(DB_PATH.read_bytes())` loaded the entire (350MB+) capture database into memory once daily just to hash it - replaced with a streaming `content_hash_file()`. All fixed, 8 new regression tests, full suite 330/330, verified live.
...
- **Phase 2 (VPS) — deliberately declined, 2026-07-29.** The project owner reviewed `VPS_MIGRATION_PLAN.md` and chose to stay on GitHub Actions' free tier rather than take on a VPS's recurring cost and operational burden while the current setup keeps working. Not a "not started yet" gap - a real decision. Revisit only if GitHub Actions' known limitations (no single persistent database, the 65-minute job ceiling, best-effort cron timing, cache growth) actually start causing problems, not preemptively.
- **Phase 7 — a real experiment protocol is now frozen, 2026-07-29.** `vb/protocol.py`'s `freeze_protocol()`/`require_active_protocol()` on a new insert-only `experiment_protocol` table, mirroring `SettlementVersion`'s supersede-not-edit pattern. Values confirmed with the project owner (not invented): **Method A only** (raw-v1, ≥3% edge — the strategy_version already running continuously in shadow mode, resolved via the same content-addressed `StrategyDefinition` row, not a parallel one), stopping rule **200 distinct settled canonical events** (or manual stop), exposure limits reused from the existing shadow config (`max_stake_per_event=3.0`, `max_stake_per_site=25.0`). `scripts/start_experiment.py` froze it for real against the live production DB (protocol id `4a94ef09-b8e6-4cdd-b7c2-b8e590fc1f4f`, `frozen_at` 2026-07-29T12:59:53Z) — verified to survive a full restore-from-cache/capture/backup cycle intact. Because Method A's strategy_version already had days of shadow decisions accumulating *before* this freeze (infrastructure testing, not results-driven tuning), those earlier decisions are explicitly excluded from being confirmatory data — `vb.evaluation_runner.run_evaluation(..., since=frozen_at)` is the new parameter that enforces this boundary in code, not just in the protocol's prose `start_rule`. Not yet wired into a recurring report (the daily `run_daily_evaluation()` still reports on full shadow history) — generating a confirmatory-scoped report is a `since=` call away whenever wanted. The v2 shadow pipeline continues to run exactly as before, ungated by this protocol.
